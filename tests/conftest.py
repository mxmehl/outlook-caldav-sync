...
def fixture_sample_json_event() -> dict[str, str]:
...
def fixture_sample_ical_event() -> Event:
    """Sample iCalendar event."""
    event = Event()
    event.add("UID", "event-1")
    event.add("SUMMARY", "Old Event")
    event.add("DTSTART", datetime(2025, 5, 20, 9, 0, tzinfo=UTC))
    event.add("DTEND", datetime(2025, 5, 20, 10, 0, tzinfo=UTC))
...
def fixture_caldav_event_mock(sample_ical_event: Event) -> MagicMock:
